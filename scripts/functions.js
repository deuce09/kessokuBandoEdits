...
export function outTransitionLandingPage(masterDiv, bbUp, bbDown) {
  ...
  bbUp.style.transition =
    "opacity 0.7s ease, transform 0.7s ease, height 0.5s ease";
  bbDown.style.transition =
    "opacity 0.7s ease, transform 0.7s ease, height 0.5s ease";
}

export function setTransitions(character) {
  character.style.transition =
    "width 0.5s cubic-bezier( 0.02, 0.98, 0.45, 0.95), transform 0.3s ease, background-image 0.5s ease, filter 0.4s ease";
}

export function setTransitionsLayer1(character) {
  character.style.transition =
    "opacity 0.3s ease, transform 0.3s ease, width 0.5s ease";
}

export function setTransitionsLayer2(character) {
  character.style.transition =
    "opacity 0.5s ease-in-out, transform 0.3s ease, width 0.5s ease";
}
...
export function hoverCharacters(
  character,
  img,
  characterLayer1,
  characterLayer2
...
export function landingPageButtonClick(
  button,
  masterDiv,
  bbUp,
  bbDown,
...
export function displayEdit(
  ...
  function descriptionValues(parent) {
    ...
    if (edit_title !== "N/A") parent.append(title, hr);
    if (edit_description !== "N/A") parent.append(desc);
    if (edit_audio !== "N/A") parent.append(audio);
    if (anime_used !== "N/A") parent.append(anime);
    if (character_used !== "N/A") parent.append(character);
    if (edit_references !== "N/A") parent.append(reference);
    if (software_used !== "N/A") parent.append(software);
    if (edit_date !== "N/A") parent.append(hr2, date);
